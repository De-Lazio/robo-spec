...
import { ArrowRight, ClipboardList, Cpu, FolderOpen, GitBranch, Settings, Users } from 'lucide-react'
...
export default function Show({ project }: { project: Project }) {
    return <AppLayout breadcrumbs={[{ label: 'Mes projets', href: '/projects' }, { label: project.name }]} actions={<Link href={`/projects/${project.id}/settings`} className="rf-button rf-button--secondary rf-button--small"><Settings size={15} />Paramètres</Link>}>
        ...
        <div className="rf-detail-grid"><section className="rf-panel"><h2>Vue d’ensemble</h2><p className="rf-description">{project.description || 'Ce projet n’a pas encore de description.'}</p><div className="rf-tags">{project.tags.length ? project.tags.map((tag) => <span key={tag}>{tag}</span>) : <span>Aucun tag</span>}</div></section><aside className="rf-panel"><h2>Indicateurs</h2><dl className="rf-metrics"><div><dt><Users size={16} />Membres</dt><dd>{project.members_count}</dd></div><div><dt><FolderOpen size={16} />Ressources</dt><dd>{project.resources_count}</dd></div><div><dt><ClipboardList size={16} />Activités</dt><dd>{project.activities_count}</dd></div></dl></aside></div>
        <section className="rf-section-heading"><div><h2>Espaces du projet</h2><p>Les modules métier seront activés au fil des prochaines phases.</p></div></section><div className="rf-module-grid"><Module icon={Users} title="Équipe" description="Invitez des membres et gérez leurs rôles." href={`/projects/${project.id}/members`} /><Module icon={ClipboardList} title="Cahier des charges" description="Définissez les 9 sections de votre spécification." href={`/projects/${project.id}/cdc`} /><Module icon={Cpu} title="Choix techniques" description="Construisez la nomenclature du projet depuis la bibliothèque." href={`/projects/${project.id}/technical-choices`} /><Module icon={FolderOpen} title="Ressources" description="Centralisez fichiers mécaniques, électroniques et logiciels." href={`/projects/${project.id}/resources`} /><Module icon={GitBranch} title="GitHub" description="Reliez le dépôt source et suivez ses activités." href={`/projects/${project.id}/github`} /></div>
    </AppLayout>
}
