...
export default function Show({ project }: { project: Project }) {
    return <AppLayout breadcrumbs={[{ label: 'Mes projets', href: '/projects' }, { label: project.name }]} actions={<Link href={`/projects/${project.id}/settings`} className="rf-button rf-button--secondary rf-button--small"><Settings size={15} />Paramètres</Link>}>
        ...
        <section className="rf-section-heading"><div><h2>Espaces du projet</h2><p>Les modules métier seront activés au fil des prochaines phases.</p></div></section><div className="rf-module-grid"><Module icon={ClipboardList} title="Cahier des charges" description="Définissez les 12 sections de votre spécification." /><Module icon={FolderOpen} title="Ressources" description="Centralisez fichiers mécaniques, électroniques et logiciels." /><Module icon={GitBranch} title="GitHub" description="Reliez le dépôt source et suivez ses activités." /></div>
    </AppLayout>
}

function Module({ icon: Icon, title, description }: { icon: typeof FolderOpen; title: string; description: string }) { return <article className="rf-module"><Icon size={20} /><h3>{title}</h3><p>{description}</p><span>Prochainement <ArrowRight size={14} /></span></article> }
