...
export default function Show({ project }: { project: Project }) {
    return <AppLayout breadcrumbs={[{ label: 'Mes projets', href: '/projects' }, { label: project.name }]} actions={<Link href={`/projects/${project.id}/settings`} className="rf-button rf-button--secondary rf-button--small"><Settings size={15} />Paramètres</Link>}>
        ...
        <section className="rf-section-heading"><div><h2>Espaces du projet</h2><p>Les modules métier seront activés au fil des prochaines phases.</p></div></section><div className="rf-module-grid"><Module icon={Users} title="Équipe" description="Invitez des membres et gérez leurs rôles." href={`/projects/${project.id}/members`} /><Module icon={ClipboardList} title="Cahier des charges" description="Définissez les 12 sections de votre spécification." href={`/projects/${project.id}/cdc`} /><Module icon={FolderOpen} title="Ressources" description="Centralisez fichiers mécaniques, électroniques et logiciels." href={`/projects/${project.id}/resources`} /><Module icon={GitBranch} title="GitHub" description="Reliez le dépôt source et suivez ses activités." href={`/projects/${project.id}/github`} /></div>
    </AppLayout>
}
